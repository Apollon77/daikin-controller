...
DaikinAC.prototype.getACDayPowerExtended = function getACDayPowerExtended(days,callback) {
    var self = this;
    this.daikinRequest.getACDayPowerExtended(days, function (err, ret, daikinResponse) {
        if (self.logger) self.logger(JSON.stringify(daikinResponse));
        if (callback) callback(err, daikinResponse);
    });
};
