...
DaikinACRequest.prototype.setCommonAdapterLED = function setCommonLED(values, callback) {
    ...
    this.restClient.post('http://' + this.ip + '/common/set_led', {data: normalizedVals, headers: {'Content-Type': 'application/x-www-form-urlencoded'}}, function (data, response) {
        parseResponse(data, 'response', callback);
    });
};

DaikinACRequest.prototype.commonRebootAdapter = function rebootAdapter(callback) {
    this.restClient.post('http://' + this.ip + '/common/reboot', {headers: {'Content-Type': 'application/x-www-form-urlencoded'}}, function (data, response) {
        parseResponse(data, 'response', callback);
    });
};
...
DaikinACRequest.prototype.setACControlInfo = function setACControlInfo(values, callback) {
    ...
    this.restClient.post('http://' + this.ip + '/aircon/set_control_info', {data: normalizedVals, headers: {'Content-Type': 'application/x-www-form-urlencoded'}}, function (data, response) {
        parseResponse(data, 'response', callback);
    });
};
    ...
    if (DaikinACTypes.fieldDef[endpoint]) {
        ...
        for (var field in responseData) {
            ...
            if (DaikinACTypes.fieldDef[endpoint][field]) {
                ...
                console.log('undefined field ' + field + ' for endpoint ' + endpoint);
            }
        }
