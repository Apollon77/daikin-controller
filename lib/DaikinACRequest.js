...
DaikinACRequest.prototype.setACSpecialMode = function setACSpecialMode(values, callback) {
    ...
    if (normalizedVals.spmode_kind === '0') {
        ...
        this.getRequest('http://' + this.ip + '/aircon/set_special_mode', vals, function (data, response) {
            parseResponse(data, 'response', callback);
        });
    }
    else {
        this.getRequest('http://' + this.ip + '/aircon/set_special_mode', normalizedVals, function (data, response) {
            parseResponse(data, 'response', callback);
        });
    }
};
...
DaikinACRequest.prototype.doGet = function doGet(url, parameters, callback) {
    ...
    if (this.logger) this.logger('Call GET ' + url + ' with ' + JSON.stringify(parameters));
    this.restClient.get(url, data, callback);
};
...
DaikinACRequest.prototype.doPost = function doPost(url, parameters, callback) {
    ...
    if (this.logger) this.logger('Call POST ' + url + ' with ' + JSON.stringify(parameters));
    this.restClient.post(url, data, callback);
};
