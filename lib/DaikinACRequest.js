...
DaikinACRequest.prototype.getACDayPowerExtended = function getACDayPowerExtended(days, callback) {
	if (days < 0 || days > 7)
		throw Error('Invalid day range');
    if (this.logger) this.logger('Call GET http://' + this.ip + '/aircon/get_day_power_ex?days' + days);
    this.getRequest('http://' + this.ip + '/aircon/get_day_power_ex?days' + days', function (data, response) {
        parseResponse(data, '/aircon/get_day_power_ex', callback);
    });
};
