...
export class DaikinDataParser {
  public static processResponse<T>(
    ...
    if (inputData instanceof Error) {
      callback?.(`Error occured: ${(inputData as Error).message}`, null, null);
      return null;
    }
    ...
    switch (ret) {
      case 'OK':
        ...
        break;
      case 'PARAM NG':
        callback?.('Wrong Parameters in request: ' + input, ret, null);
        return null;
      case 'ADV NG':
        callback?.('Wrong ADV: ' + input, ret, null);
        return null;
      default:
        callback?.('Unknown response: ' + input, ret, null);
        return null;
    }
    return dict;
  }
